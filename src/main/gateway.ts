...
export function gatewayShim(model: string, claudeExecutable: string): string | null {
  ...
  /*
   * The helper model, pinned rather than left on `auto`.
   *
   * `auto` is the wrapper's default mapping, and it resolves `haiku` and every
   * background helper to DeepSeek V4 Flash. That model answers Claude Code's
   * background prompts with a single token, every time, so whatever asked keeps
   * asking: 141,485 requests in one day, all `output_tokens: 1` against ~3,400
   * input tokens, 546M input tokens for 514K of output. Seventy to three hundred
   * and seventy background calls per real turn, against a shared gateway quota
   * that then returns 429 - which reads in the app as "Claude is rate limiting
   * me" and is nothing of the sort.
   *
   * Pinning the helper to the session's own model resolves every
   * Haiku/Sonnet/Opus/background alias to a model that can actually answer.
   *
   * Read the tradeoff before changing this. The flag is the wrapper's
   * highest-precedence subagent override, and under `auto` the wrapper
   * deliberately leaves `CLAUDE_CODE_SUBAGENT_MODEL` unset so per-invocation
   * `Agent` model choices keep working. Pinning takes that away: a `Task`
   * fan-out that used to pick a cheap tier now runs every subagent on the
   * session's model. On an Opus session that is not cheap. The trade is
   * deliberate - 139,803 wasted calls and a 429'd session is worse than
   * expensive subagents - but it is a trade, not a free win.
   *
   * Also unverified: the causal link from the one-token replies to the retry
   * volume is inferred from the shape of the traffic, not observed. Measure it.
   * Run one turn each way and compare, from the proxy log:
   *   ATELIER_GATEWAY_HELPER_MODEL=auto   # the old behaviour
   * then count `messages request` lines and one-token replies per real turn.
   * `auto` also reverts this entirely if the trade turns out to be wrong.
   */
  const helper = envVar('GATEWAY_HELPER_MODEL') || name
  ...
  const body = `${[
    ...
    `  --aigw-subagent-model ${shellQuote(helper)} \\`,
